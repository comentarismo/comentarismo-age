...
type AgeStruct struct {
	...
	Age      string `json:"age"`
	Language string `json:"age"`
}

var REDIS_HOST = os.Getenv("REDIS_HOST")
var REDIS_PORT = os.Getenv("REDIS_PORT")
var REDIS_PASSWORD = os.Getenv("REDIS_PASSWORD")
var AGE_DEBUG = os.Getenv("AGE_DEBUG")

var LEARNAGE = os.Getenv("LEARNAGE")

var AgeList []AgeStruct
...
func WriteToFile(name string) (err error) {
	...
	err = enc.Encode(AgeList)
	return
}

// De-Serialize this classifier from a file.
func ReadFromFile(name string) (AgeList []AgeStruct, err error) {
	...
	err = dec.Decode(&AgeList)
	return
}

func LearnFromFile(name string) (AgeList []AgeStruct, err error) {
	AgeList, err = ReadFromFile(name)
	if err != nil {
		log.Println("Error when LearnFromFile() ReadFromFile(), ", err)
		return
	}
	for _, v := range AgeList {
		Train(v.Age, v.Name, v.Language)
	}
	return
}
...
func Classify(text, lang string) (key string) {
	...
	if scores != nil {
		...
		//if key == "bad" && max == 0 {
		//	Debug("Will reclassify false age to not age as score is too low for being age ")
		//	key = "good"
		//}
...
func StartLanguageAge(age_range, filename string, done chan bool) {
	Debug("StartLanguageAge init, ", filename)
	...
	for _, record := range result {
		...
		//idx := 0
		//for idx <= int(count) {
		Debug("Train(age_range, name, lang)", age_range, name, "en")
		Train(age_range, name, "en")
		//idx++
		//}
		//
		AgeList = append(AgeList, AgeStruct{
			Count:    count,
			Name:     name,
			Age:      age_range,
			Language: "en",
