...
func Test_getRuns(t *testing.T) {
	...
	runs = lang.GetRuns(words)

	if !lang.KeyExists("Basic Latin", runs) {
		t.Errorf("Runs must contain 'Basic Latin'")
	}

	words = strings.Split("abcdééé", " ")
	runs = lang.GetRuns(words)
	if len(runs) != 2 || !lang.KeyExists("Basic Latin", runs) || !lang.KeyExists("Extended Latin", runs) {
	...
	runs = lang.GetRuns(words)
	if !lang.KeyExists("Cyrillic", runs) {
...
func Test_GetOrderedModel(t *testing.T) {
	...
	if len(om) != 1 || !lang.KeyExists("abc", om) {
		t.Errorf("Model must be [abc]")
	}

	om = models.GetOrderedModel("abca")
	if len(om) != 2 || !lang.KeyExists("abc", om) || !lang.KeyExists("bca", om) {
		t.Errorf("Model must be [abc bca]")
	}

	om = models.GetOrderedModel("abcabdcab")
	if len(om) != 6 ||
		!lang.KeyExists("cab", om) ||
		!lang.KeyExists("abc", om) ||
		!lang.KeyExists("abd", om) ||
		!lang.KeyExists("bca", om) ||
		!lang.KeyExists("bdc", om) ||
		!lang.KeyExists("dca", om) {
...
func Test_Guess(t *testing.T) {
	...
	if lang.GuessId(french) != lang.IanaMap["fr"] {
